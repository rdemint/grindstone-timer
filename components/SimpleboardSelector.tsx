...
export default function SimpleboardSelector({ currentHand, setHand}: {currentHand: IHand, setHand: Function}) {
    ...
    return (
        <div className='flex flex-col bg-slate-700 text-slate-50 md:w-24 rounded'>
            <button onClick={() => setHand({...currentHand, hangboard: simpleboard, jug})} className={`bg-slate-400 h-8 px-2 rounded-md ${currentHand.hangboard.name === simpleboard.name && currentHand.hold.name === 'jug' ? 'bg-green-400 text-slate-800' : ''}`}>Jug</button>
